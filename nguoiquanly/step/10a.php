...
<?php
  ...
  while($rows = mysql_fetch_assoc($sql))
  {
    ...
    $id_import_data     = SHOW_text($rows['id_import_data']);
...
?>
      <tr>
        ...
        <td>
          <div class="name">
            <input type="text" name="ncata_vi<?=$cl?>" class="cls_emty_name" value="<?=$tenbaiviet_vi?>" placeholder="Tiêu đề vi">
          </div>
          <?php if($lang_en){ ?>
          <div class="name">
            <input type="text" name="ncata_en<?=$cl?>" class="cls_emty_name" value="<?=$tenbaiviet_en?>" placeholder="Tiêu đề en">
          </div>
          <?php } ?>
          <?php if(isset($_SESSION['admin'])){ ?>
          <div class="name">
            <select name="id_parent<?=$cl?>">
              <option value="0">Chọn danh mục</option>
              <?php foreach ($list_danhmuc as $key => $value) { if($value["id_parent"] != 0 || $value['step'] != 9) continue; ?>
              <option value="<?=$key ?>" <?=$id_parent == $key ? 'selected="selected"'  : "" ?>><?=$value["tenbaiviet_vi"] ?></option>
              <?php } ?>
            </select>
          </div>
          <?php } ?>
