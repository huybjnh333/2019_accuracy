            ...
            <div class="tab-pane active" id="tab_1">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control"
                           value='<?= !empty($tenbaiviet_vi) ? $tenbaiviet_vi : '' ?>' name="tenbaiviet_vi"
                           id="tenbaiviet_vi">
                </div>
<!--                <div class="form-group">-->
...
<!--                              cols="50">-->
<!--                --><?//= !empty($mota_vi) ? SHOW_text($mota_vi) : '' ?>
<!--                        </textarea>-->
<!--                </div>-->
...
<!--                              cols="80">-->
<!--            --><?//= !empty($mt_2_vi) ? SHOW_text($mt_2_vi) : '' ?><!--</textarea>-->
<!--                </div>-->
                ...
                <!--                <div class="form-group">-->
                <!--                    <label>Tags </label>-->
                <!--                    <input type="text" class="form-control" name="tags_vi"-->
                <!--                           value="--><? //= !empty($tags_vi) ? Show_text($tags_vi) : "" ?><!--">-->
                <!--                </div>-->
            </div>
            <?php if ($lang_nb2) { ?>
...
<!--                    <div class="form-group">-->
<!--                        <label>Chi Tiết Sản Phẩm(--><?//= _lang_nb2_key ?><!--)</label>-->
<!--                        <textarea id="mota_en" name="mota_en" rows="10"-->
<!--                                  cols="50">-->
<!--                    --><?//= !empty($mota_en) ? SHOW_text($mota_en) : '' ?>
<!--                    </textarea>-->
<!--                    </div>-->
<!--                    <div class="form-group">-->
<!--                        <label>Ứng Dụng (--><?//= _lang_nb2_key ?><!--)</label>-->
<!--                        <textarea id="mt_2_en" name="mt_2_en" rows="10"-->
<!--                                  cols="80">-->
<!--                --><?//= !empty($mt_2_en) ? SHOW_text($mt_2_en) : '' ?><!--</textarea>-->
<!--                    </div>-->
                    ...
                    <!--                                  cols="80">-->
                    <? //= !empty($mt_3_en) ? SHOW_text($mt_3_en) : '' ?><!--</textarea>-->
                    <!--                    </div>-->
                    ...
                    <!--                    <div class="form-group">-->
                    <!--                        <label>Tags(--><? //= _lang_nb2_key ?><!--)</label>-->
                    <!--                        <input type="text" class="form-control" name="tags_en"-->
                    <!--                               value="-->
                    <? //= !empty($tags_en) ? Show_text($tags_en) : "" ?><!--">-->
                    <!--                    </div>-->
                </div>
            <?php } ?>
            <?php if ($lang_nb3) { ?>
    ...
    <div class="box p10 dv-tinhnang-ok">
        <?php

        $tinhnang = LAY_bv_tinhnang($_GET['step']);
                    ...
                    foreach ($tinhnang as $val2) {
                        ...
                        <label>
                            <input type="checkbox" name="detail_vi[]"
                                   value="<?= $val2['id'] ?>" <?= in_array($val2['id'], $detail_vi) ? "checked" : "" ?>> <?= $val2['tenbaiviet_vi'] ?>
                        </label>
                    <?php } ?>
                    <div class="clear"></div>
                </div>
    ...
    <div class="box p10">
       <!-- <div class="form-group">
            ...
            <input type="text" class="form-control" name="youtubelink"
                   value="<?= !empty($youtubelink) ? Show_text($youtubelink) : "" ?>">
        </div>-->
        ...
        <?php } ?>
        <?php if (CHECK_key_setting("gia-san-pham")) { ?>
            <div class="form-group">
        ...
        <?php } ?>
        <?php if (CHECK_key_setting("san-pham-khuyen-mai")) { ?>
            <div class="form-group">
                <label>Giá khuyến mãi: </label>
                <input type="text" class="form-control cls_giatien_khuyenmai_f" name="giakm"
                       value="<?= !empty($giakm) ? Show_text($giakm) : "0" ?>" onkeyup="SetCurrency(this)">
                <label style=" margin-top: 10px; margin-bottom: 0;"><input type="checkbox" class="minimal" name="opt_km"
                                                                           value="1" <?= LAY_checked(@$opt_km, 1) ?>> Áp
                    dụng KM</label>
            </div>
        <?php } ?>
        ...
        <?php if (CHECK_key_setting("san-pham-hover")) { ?>
            <div class="form-group">
                <label for="exampleInputFile2">Hình đại diện
                    hover <?= !empty($thongtin_step['size_img']) && $thongtin_step['size_img'] != '' ? "(" . str_replace("x", "px x ", $thongtin_step['size_img']) . "px)" : '' ?></label>
        ...
        if (in_array($_GET['step'], $array_dm)) {
            ?>
            <div class="form-group">
                <label>Thuộc chủ đề</label>
                <?= LAY_chude($id_parent, $_GET['step'], 'id_parent', 'form-control', 0, $_GET['id_step'], 0, 'true', 1) ?>
            </div>
        <?php } ?>
        ...
        <div class="form-group">
            ...
            <div class="input-group date">
                ...
                <input name="ngaydang" type="text" class="form-control pull-right" id="datepicker"
                       value='<?= $ngaydang ?>'>
            </div>
        </div>
        <div class="form-group">
