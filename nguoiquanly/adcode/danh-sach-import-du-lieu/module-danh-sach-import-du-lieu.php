    ...
    if(isset($_REQUEST['addgiatri']) AND isset($_REQUEST['maxvalu']))
    {
      for($i=0;$i<$_REQUEST['maxvalu'];$i++)
      {
          ...
          //
            ...
            DB_queDB_que("UPDATE `$table` SET `ten_vi`='$ten_vi' WHERE `id`='$idofme' limit 1");
          }
      }
                      ...
                      <table class="table table-hover table-danhsach">
                        <tbody>
                          <tr>
                            ...
                            <th class="w200 text-center">Ngày đăng</th>
                    ...
                    ?>
                          <tr>
                            ...
                            <td >
                              ...
                              <p style="font-size: 12px; margin-top: 3px; color: #989797;">Số lần import <b><?=$so_lan_import ?></b>. <?=($import_cuoi != 0) ? date('d-m-Y H:i:s', $import_cuoi) : "" ?></p>
                            </td>
                            <td class="text-center">
...
<?php } ?>
<style type="text/css">
  .dv-load-import .dv-nd-import-dulieu-cont { height: 145px; overflow-y: auto; border: 1px solid #e2e2e2; padding: 7px; margin-top: 10px; }
  .dv-load-import-cont { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.38); z-index: 10000; display: none}
  .dv-load-import { width: 500px; height: 200px; position: fixed; background: #fff; left: 50%; margin-left: -250px; top: 50%; margin-top: -150px; padding: 12px; z-index: 10001; display: none; border-radius: 5px; }
...
<div class="dv-load-import">
  <h3><span class="img"></span><span class="con">Bắt đầu import dữ liệu...</span><span class="ip_total"></span></h3>
      ...
      if(cf){
        ...
        $.ajax({
          ...
          success: function(data){
            ...
            $(".dv-load-import h3 .con").html('<i class="fa fa-pencil-square-o"></i>Hoàn thành import dữ liệu <a class="cur" onclick="DONG_import()"><i class="fa fa-times"></i></a>');
          }
        });
      }
    }
