...
if (isset($_GET['them-moi']) || (isset($_GET['edit']) && is_numeric($_GET['edit']))) {
    include "module-danh-sach-lien-ket-nhanh-add.php";
} else {
    ...
    if (isset($_REQUEST['addgiatri']) AND isset($_REQUEST['maxvalu'])) {
        for ($i = 1; $i <= $_REQUEST['maxvalu']; $i++) {
            ...
            $lien_ket_vi = isset($_POST["lien_ket_vi$i"]) ? $_POST["lien_ket_vi$i"] : '';
            $lien_ket_en = isset($_POST["lien_ket_en$i"]) ? $_POST["lien_ket_en$i"] : '';
            ...
            if (isset($_POST["xoa_gr_arr_$i"])) {
                ...
                DB_que("UPDATE `$table` SET `tenbaiviet_vi`='$tenbaiviet_vi', `tenbaiviet_en`='$tenbaiviet_en', `lien_ket_vi`='$lien_ket_vi', `lien_ket_en`='$lien_ket_en', `target`='$target', `catasort`='$sort', `showhi`='$showhi' WHERE `id`='$idofme' LIMIT 1");
            }
        }
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active">Danh sách liên kết nhanh</li>
        </ol>
    </section>
                ...
                <section class="col-lg-12">
                    ...
                    <div class="box">
                        <div class="box-header">
                            <h2 class="h2_title">
                                <i class="fa fa-pencil-square-o"></i> Liên kết nhanh
                            </h2>
                        ...
                        <div class="box-body table-responsive no-padding table-danhsach-cont">
                            <table class="table table-hover table-danhsach">
                                ...
                                <tr>
                                    ...
                                    <th>Tiêu đề</th>
                                    <!--                            <th class="w200 text-center">Target</th>-->
                                ...
                                foreach ($sql_array as $rows) {
                                    ...
                                    $lien_ket_vi = SHOW_text($rows['lien_ket_vi']);
                                    $lien_ket_en = SHOW_text($rows['lien_ket_en']);
                                    $target = SHOW_input($rows['target']);
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="name">
                                                <input type="text" class="cls_emty_name" name="tenbaiviet_vi<?= $cl ?>"
                                                       value="<?= $tenbaiviet_vi ?>"
                                                       placeholder="Tiêu đề (<?= _lang_nb1_key ?>)">
                                            </div>
                                            <?php if ($lang_nb2) { ?>
                                                <div class="name">
                                                    <input type="text" class="cls_emty_name"
                                                           name="tenbaiviet_en<?= $cl ?>" value="<?= $tenbaiviet_en ?>"
                                                           placeholder="Tiêu đề (<?= _lang_nb2_key ?>)">
                                                </div>
                                            <?php } ?>

                                            <div class="name">
                                                <input type="text" class="cls_emty_name" name="lien_ket_vi<?= $cl ?>"
                                                       value="<?= $lien_ket_vi ?>"
                                                       placeholder="Liên kết (<?= _lang_nb1_key ?>)">
                                            </div>
                                            <?php if ($lang_nb2) { ?>
                                                <div class="name">
                                                    <input type="text" class="cls_emty_name"
                                                           name="lien_ket_en<?= $cl ?>" value="<?= $lien_ket_en ?>"
                                                           placeholder="Liên kết (<?= _lang_nb2_key ?>)">
                                                </div>
                                            <?php } ?>
                                        ...
                                        <!-- <td class="text-center">
                              <select name="target<?= $cl ?>" class="form-control">
                                <option value="">Mặc định</option>
                                <option value="_blank" <?= !empty($target) && $target == '_blank' ? 'selected="selected"' : '' ?> >Cửa sổ mới</option>
                              </select>
                            </td>-->
